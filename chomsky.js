...
function minimize_loop(tree, minArr) {
  ...
  for (var i = 0; i < minArr.length; i++) {
    ...
    // if (i == minArr.length - 1 && level === 0) {
      // minimize_list.pop();
      // minimize_list.push({ 'func': minimize_96, 'rule': "AB+AC -> A(B+C) Factor", 'type': '' });
      // i = -1;
      // level = 1;
    // }
...
minimize_list.push({ 'func': minimize_03, 'rule': "(AB)C -> ABC", 'type': '' });
minimize_list.push({ 'func': minimize_04, 'rule': "A+(B+C) -> A+B+C", 'type': '' });
minimize_list.push({ 'func': minimize_05, 'rule': "(A+B)+C -> A+B+C", 'type': '' });
minimize_list.push({ 'func': minimize_06, 'rule': "λ+AA* -> A*", 'type': '' });
minimize_list.push({ 'func': minimize_07, 'rule': "A...+A* -> A*", 'type': '' });
...
minimize_list.push({ 'func': minimize_12, 'rule': "(A+λ)* -> (A)*", 'type': '' });
minimize_list.push({ 'func': minimize_13, 'rule': "A*(BA*)* -> (A+B)*", 'type': '' });
minimize_list.push({ 'func': minimize_14, 'rule': "(A*B)*A* -> (A+B)*", 'type': '' });
minimize_list.push({ 'func': minimize_15, 'rule': "λA -> A", 'type': '' });
minimize_list.push({ 'func': minimize_16, 'rule': "$+A* -> A*", 'type': '' });
minimize_list.push({ 'func': minimize_17, 'rule': "A+A -> A", 'type': '' });
minimize_list.push({ 'func': minimize_18, 'rule': "A+A* -> A*", 'type': '' });
minimize_list.push({ 'func': minimize_19, 'rule': "A*A* -> A*", 'type': '' });
minimize_list.push({ 'func': minimize_20, 'rule': "(AA+A)* -> (A)*", 'type': '' });
minimize_list.push({ 'func': minimize_21, 'rule': "A*AA* -> AA*", 'type': '' });
minimize_list.push({ 'func': minimize_22, 'rule': "(λ+(A+B)*A)B* -> (A+B)*", 'type': '' });
minimize_list.push({ 'func': minimize_23, 'rule': "A*(λ+B(A+B)*) -> (A+B)*", 'type': '' });
minimize_list.push({ 'func': minimize_24, 'rule': "A*(A+B)* -> (A+B)*", 'type': '' });
minimize_list.push({ 'func': minimize_25, 'rule': "(A+B)*A* -> (A+B)*", 'type': '' });
minimize_list.push({ 'func': minimize_26, 'rule': "(λ+A)A*(λ+A) -> A*", 'type': '' });
minimize_list.push({ 'func': minimize_27, 'rule': "(λ+A+B)(A+B)*(λ+A+B) -> (A+B)*", 'type': '' });

minimize_list.push({ 'func': minimize_28, 'rule': "AB+AC -> A(B+C)", 'type': '' });
minimize_list.push({ 'func': minimize_29, 'rule': "AB+CB -> (A+C)B", 'type': '' });

minimize_list.push({ 'func': minimize_30, 'rule': "λ+(A+B)*B -> (A*B)*", 'type': '' });
minimize_list.push({ 'func': minimize_31, 'rule': "λ+A(A+B)* -> (AB*)*", 'type': '' });

minimize_list.push({ 'func': minimize_32, 'rule': "A+BB*A -> (λ+B*B)A", 'type': '' });
minimize_list.push({ 'func': minimize_33, 'rule': "A+B*BA -> (λ+B*B)A", 'type': '' });
minimize_list.push({ 'func': minimize_34, 'rule': "BB*A+A -> (BB*+λ)A", 'type': '' });
minimize_list.push({ 'func': minimize_35, 'rule': "B*BA+A -> (B*B+λ)A", 'type': '' });
minimize_list.push({ 'func': minimize_36, 'rule': "A+ABB* -> A(λ+BB*)", 'type': '' });
minimize_list.push({ 'func': minimize_37, 'rule': "A+AB*B -> A(λ+B*B)", 'type': '' });
minimize_list.push({ 'func': minimize_38, 'rule': "ABB*+A -> A(BB*+λ)", 'type': '' });
minimize_list.push({ 'func': minimize_39, 'rule': "AB*B+A -> A(B*B+λ)", 'type': '' });
minimize_list.push({ 'func': minimize_40, 'rule': "A*+A*BB*A* -> A*(λ+BB*)A*", 'type': '' });
...
minimize_list.push({ 'func': minimize_fsm03, 'rule': "A*B* -> B* IF A*⊆B*", 'type': '' });

// minimize_list.push({ 'func': minimize_90, 'rule': "A+AB -> A(λ+B)", 'type': '' });
// minimize_list.push({ 'func': minimize_91, 'rule': "AB+A -> A(B+λ)", 'type': '' });
// minimize_list.push({ 'func': minimize_92, 'rule': "AB+B -> (A+λ)B", 'type': '' });
// minimize_list.push({ 'func': minimize_93, 'rule': "A+BA -> (λ+B)A", 'type': '' });
    ...
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (!tree.add[i].isMul()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (tree.add[j].isMul() && tree.add[j].mul.length > 3) {
            if (areEqual(arrLast(tree.add[j].mul), tree.add[i])) {
              var starList = hasStar(tree.add[j].mul);
...
function starAdjust(arrMul, myStar) {
  // BB(BB)*A -> (BB)(BB)*A
  
  var finalMul = [];
  
  for (var i = 0; i < arrMul.length - 1; i++) {
    if (i == myStar) {
      continue;
    }
...
function minimize_43(tree) {
  ...
  if (tree.isAdd() && tree.add.length >= 2) {
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (tree.add[i].isMul()
          && tree.add[i].mul.length === 4
          && tree.add[i].mul[1].isStar()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (tree.add[j].isStar()) {
            if (areEqual(arrLast(tree.add[i].mul), tree.add[j])
                && areEqual(tree.add[i].mul[0], tree.add[j])) {
              if (areEqual(tree.add[i].mul[2], tree.add[i].mul[1].star)) {
                var _inMul = genMul(tree.add[i].mul.slice(1, tree.add[i].mul.length - 1));
                var _add = genAdd([_inMul, genLam()]);
                var _mul = genMul([tree.add[j], _add, tree.add[j]]);
...
function minimize_42(tree) {
  ...
  if (tree.isAdd() && tree.add.length >= 2) {
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (tree.add[i].isMul()
            && tree.add[i].mul.length === 4
            && tree.add[i].mul[2].isStar()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (tree.add[j].isStar()) {
            if (areEqual(arrLast(tree.add[i].mul), tree.add[j])
                && areEqual(tree.add[i].mul[0], tree.add[j])) {
              if (areEqual(tree.add[i].mul[1], tree.add[i].mul[2].star)) {
                var _inMul = genMul(tree.add[i].mul.slice(1, tree.add[i].mul.length - 1));
                var _add = genAdd([_inMul, genLam()]);
                var _mul = genMul([tree.add[j], _add, tree.add[j]]);
  ...
  if (tree.isAdd() && tree.add.length >= 2) {
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (tree.add[i].isStar()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (tree.add[j].isMul()
              && tree.add[j].mul.length === 4
              && tree.add[j].mul[1].isStar()) {
            if (areEqual(arrLast(tree.add[j].mul), tree.add[i])
                && areEqual(tree.add[j].mul[0], tree.add[i])) {
              if (areEqual(tree.add[j].mul[2], tree.add[j].mul[1].star)) {
                var _inMul = genMul(tree.add[j].mul.slice(1, tree.add[j].mul.length - 1));
                var _add = genAdd([genLam(), _inMul]);
                var _mul = genMul([tree.add[i], _add, tree.add[i]]);
    ...
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (tree.add[i].isStar()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (tree.add[j].isMul()
              && tree.add[j].mul.length === 4
              && tree.add[j].mul[2].isStar()) {
            if (areEqual(arrLast(tree.add[j].mul), tree.add[i])
                && areEqual(tree.add[j].mul[0], tree.add[i])) {
              if (areEqual(tree.add[j].mul[1], tree.add[j].mul[2].star)) {
                var _inMul = genMul(tree.add[j].mul.slice(1, tree.add[j].mul.length - 1));
...
function minimize_39(tree) {
  ...
  if (tree.isAdd() && tree.add.length >= 2) {
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (tree.add[i].isMul()
          && tree.add[i].mul.length == 3
          && tree.add[i].mul[1].isStar()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (!tree.add[j].isMul()) {
            if (areEqual(tree.add[i].mul[0], tree.add[j])) {
              if (areEqual(arrLast(tree.add[i].mul[1].star, arrLast(tree.add[i].mul)))) {
                var _inMul = genMul(tree.add[i].mul.slice(1, tree.add[i].mul.length));
                var _add = genAdd([_inMul, genLam()]);
                var _mul = genMul([tree.add[j], _add]);
...
function minimize_38(tree) {
  ...
  if (tree.isAdd() && tree.add.length >= 2) {
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (tree.add[i].isMul()
          && tree.add[i].mul.length == 3
          && arrLast(tree.add[i].mul).isStar()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (!tree.add[j].isMul()) {
            if (areEqual(tree.add[i].mul[0], tree.add[j])) {
              if (areEqual(arrLast(tree.add[i].mul).star, tree.add[i].mul[1])) {
                var _inMul = genMul(tree.add[i].mul.slice(1, tree.add[i].mul.length));
                var _add = genAdd([_inMul, genLam()]);
                var _mul = genMul([tree.add[j], _add]);
  ...
  if (tree.isAdd() && tree.add.length >= 2) {
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (!tree.add[i].isMul()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (tree.add[j].isMul()
          && tree.add[j].mul.length == 3
          && tree.add[j].mul[1].isStar()) {
            if (areEqual(tree.add[j].mul[0], tree.add[i])) {
              if (areEqual(arrLast(tree.add[j].mul[1].star, arrLast(tree.add[j].mul)))) {
                var _inMul = genMul(tree.add[j].mul.slice(1, tree.add[j].mul.length));
                var _add = genAdd([genLam(), _inMul]);
                var _mul = genMul([tree.add[i], _add]);
    ...
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (!tree.add[i].isMul()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (tree.add[j].isMul()
          && tree.add[j].mul.length == 3
          && arrLast(tree.add[j].mul).isStar()) {
            if (areEqual(tree.add[j].mul[0], tree.add[i])) {
              if (areEqual(arrLast(tree.add[j].mul).star, tree.add[j].mul[1])) {
                var _inMul = genMul(tree.add[j].mul.slice(1, tree.add[j].mul.length));
                var _add = genAdd([genLam(), _inMul]);
                var _mul = genMul([tree.add[i], _add]);
...
function minimize_35(tree) {
  ...
  if (tree.isAdd() && tree.add.length >= 2) {
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (tree.add[i].isMul() && tree.add[i].mul.length == 3
              && tree.add[i].mul[0].isStar()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (!tree.add[j].isMul()) {
            if (areEqual(arrLast(tree.add[i].mul), tree.add[j])) {
              if (areEqual(tree.add[i].mul[0].star, tree.add[i].mul[1])) {
                var _inMul = genMul(tree.add[i].mul.slice(0, tree.add[i].mul.length - 1));
                var _add = genAdd([_inMul, genLam()]);
                var _mul = genMul([_add, tree.add[j]]);
    ...
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (tree.add[i].isMul() && tree.add[i].mul.length == 3
        ...
        for (var j = i + 1; j < tree.add.length; j++) {
          if (!tree.add[j].isMul()) {
            if (areEqual(arrLast(tree.add[i].mul), tree.add[j])) {
              if (areEqual(tree.add[i].mul[1].star, tree.add[i].mul[0])) {
                var _inMul = genMul(tree.add[i].mul.slice(0, tree.add[i].mul.length - 1));
                var _add = genAdd([_inMul, genLam()]);
                var _mul = genMul([_add, tree.add[j]]);
  ...
  if (tree.isAdd() && tree.add.length >= 2) {
    for (var i = 0; i < tree.add.length - 1; i++) {
      if (!tree.add[i].isMul()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (tree.add[j].isMul() && tree.add[j].mul.length == 3
              && tree.add[j].mul[0].isStar()) {
            if (areEqual(arrLast(tree.add[j].mul), tree.add[i])) {
              if (areEqual(tree.add[j].mul[0].star, tree.add[j].mul[1])) {
                var _inMul = genMul(tree.add[j].mul.slice(0, tree.add[j].mul.length - 1));
                var _add = genAdd([genLam(), _inMul]);
                var _mul = genMul([_add, tree.add[i]]);
      ...
      for (var i = 0; i < tree.add.length; i++) {
        ...
        if (tree.add[i].isMul() && tree.add[i].mul.length == 2) {
          if (tree.add[i].mul[1].isStar() 
              && tree.add[i].mul[1].star.isAdd()
              && tree.add[i].mul[1].star.add.length == 2) {
            
            if (areEqual(tree.add[i].mul[1].star.add[0], tree.add[i].mul[0])) {
              var starNode = tree.add[i].mul[1].star.add[1];
              var nStarNode = tree.add[i].mul[1].star.add[0];
              
              var _inStar = genStar(starNode);
              var _mul = genMul([nStarNode, _inStar]);
              var _star = genStar(_mul);
              
              if (lamIdx > i) {
                tree.add.splice(lamIdx, 1);
                tree.add.splice(i, 1, _star);                
                
              } else {
                tree.add.splice(i, 1);     
                tree.add.splice(lamIdx, 1, _star);
              }
              
              return true;
              
            } else if (areEqual(tree.add[i].mul[1].star.add[1], tree.add[i].mul[1])) {
              var starNode = tree.add[i].mul[1].star.add[0];
              var nStarNode = tree.add[i].mul[1].star.add[1];
  ...
  if (tree.isAdd() && tree.add.length >= 2) {
    ...
    if (lamIdx > -1) {
      for (var i = 0; i < tree.add.length; i++) {
        ...
        if (tree.add[i].isMul() && tree.add[i].mul.length == 2) {
          if (tree.add[i].mul[0].isStar() 
              && tree.add[i].mul[0].star.isAdd()
              && tree.add[i].mul[0].star.add.length == 2) {
            
            if (areEqual(tree.add[i].mul[0].star.add[0], tree.add[i].mul[1])) {
              var starNode = tree.add[i].mul[0].star.add[1];
              var nStarNode = tree.add[i].mul[0].star.add[0];
              
              var _inStar = genStar(starNode);
              var _mul = genMul([_inStar, nStarNode]);
              var _star = genStar(_mul);
            ...
            } else if (areEqual(tree.add[i].mul[0].star.add[1], tree.add[i].mul[1])) {
              var starNode = tree.add[i].mul[0].star.add[0];
              var nStarNode = tree.add[i].mul[0].star.add[1];
      ...
      if (tree.mul[i].isAdd() && tree.mul[i + 1].isStar() && tree.mul[i + 2].isAdd()) {
        if (tree.mul[i].add.length == 2 && tree.mul[i + 2].add.length == 2) {
          if (contains(tree.mul[i].add, genLam())
              ...
              && contains(tree.mul[i + 2].add, genLam())
              && contains(tree.mul[i + 2].add, tree.mul[i + 1].star)){
            tree.mul.splice(0, 1);
            tree.mul.splice(-1, 1)
    ...
    for (var i = 0; i < tree.add.length; i++) {
      ...
      // one of them are not multiply
        ...
        if (tree.add[nMulIdx].isStar()
          ...
          var _add = genAdd([rest1, rest2]);
          var _mul = genMul([common, _add, common]);
          tree.add[nMulIdx] = _mul;
...
function minimize_94(tree) {
  // A(B+C) -> AB+AC Distribute
        ...
        for (var j = i + 1; j < tree.add.length; j++) {
          if (tree.add[j].isMul() && tree.add[j].mul.length == 3) {
            if (areEqual(arrLast(tree.add[j].mul), tree.add[i])) {
              ...
              if (tree.add[j].mul[0].isStar()) {
                first = tree.add[j].mul[0].star;
                last = tree.add[j].mul[1];
                hasStar = true;
              }
              
              else if (tree.add[j].mul[1].isStar()) {
                last = tree.add[j].mul[1].star;
                first = tree.add[j].mul[0]
                hasStar = true;
              }
        ...
        for (var j = i + 1; j < tree.add.length; j++) {
          if (!tree.add[j].isMul()) {
            if (areEqual(arrLast(tree.add[i].mul), tree.add[j])) {
              ...
              if (tree.add[i].mul[0].isStar()) {
                first = tree.add[i].mul[0].star;
                last = tree.add[i].mul[1];
                hasStar = true;
                
              } else if (tree.add[i].mul[1].isStar()) {
                last = tree.add[i].mul[1].star;
                first = tree.add[i].mul[0]
                hasStar = true;
              }
  ...
  if (tree.isAdd() && tree.add.length >= 2) {
    for (var i=0; i<tree.add.length-1; i++) {
      if (!tree.add[i].isMul()) {
        for (var j = i + 1; j < tree.add.length; j++) {
          if (tree.add[j].isMul() && tree.add[j].mul.length == 3) {
            if (areEqual(tree.add[i], tree.add[j].mul[0])) {
              var first = null;
              var last = null;
              var hasStar = false;
              
              if (arrLast(tree.add[j].mul).isStar()) {
                first = arrLast(tree.add[j].mul).star;
                last = arrLast(tree.add[j].mul, 2);
                hasStar = true;
                
              } else if (arrLast(tree.add[j].mul, 2).isStar()) {
                last = arrLast(tree.add[j].mul, 2).star;
                first = arrLast(tree.add[j].mul);
                hasStar = true;
              }
              ...
              if (areEqual(first, last) && hasStar) {
                var first = tree.add[i];
                var rest1 = genLam();
                var rest2 = genMul(tree.add[j].mul.slice(1, tree.add[j].mul.length));
        ...
        for (var j = i + 1; j < tree.add.length; j++) {
          if (!tree.add[j].isMul()) {
            if (areEqual(tree.add[i].mul[0], tree.add[j])) {
              ...
              if (arrLast(tree.add[i].mul).isStar()) {
                first = arrLast(tree.add[i].mul).star;
                last = arrLast(tree.add[i].mul, 2);
                hasStar = true;
                
              } else if (arrLast(tree.add[i].mul, 2).isStar()) {
                last = arrLast(tree.add[i].mul, 2).star;
                first = arrLast(tree.add[i].mul);
                hasStar = true;
              }

              if (areEqual(first, last) && hasStar) {
                var first = tree.add[j];
                var rest1 = genMul(tree.add[i].mul.slice(1, tree.add[i].mul.length));
                var rest2 = genLam();
